...
import {Directive, Input,
  OnDestroy, AfterViewInit, ElementRef, Optional, InjectionToken, Injectable, Inject, Provider} from '@angular/core';
...
export class CdkStickyHeader implements OnDestroy, AfterViewInit {
  ...
  constructor(element: ElementRef,
              ...
              platform: Platform,
              @Inject(STICKY_HEADER_SUPPORT_STRATEGY) public _isPositionStickySupported: boolean) {
    if (platform.isBrowser) {
      console.log(_isPositionStickySupported);
  ...
  private _setStrategyAccordingToCompatibility(): void {
    ...
    // this._isPositionStickySupported = this._check.getSupport();
    // this._isPositionStickySupported = this._isPositionStickySupported;
    console.log('+++++++++' + this._isPositionStickySupported);
    if (this._isPositionStickySupported) {
   ...
   * It decides when a header should be stick and when should it be unstuck by
   * comparing the offsetTop of scrollable container with the top and bottom of
   * the sticky region.
   */
